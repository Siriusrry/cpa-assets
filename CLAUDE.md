...
- Maintain two functional customizations across upstream releases: Codex priority badges and email quota-card/window labels with full filenames retained on hover; read the supplied email directly without missing-email branches. Use only `<official-version>-Siriusrry` for display and release tags, with no extra revision suffix.
