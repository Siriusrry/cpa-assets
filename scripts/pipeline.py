...
def release_tag(tag, revision):
    if not re.fullmatch(r'v\d+\.\d+\.\d+', tag):
        raise ValueError(f'Unrecognized stable upstream tag: {tag!r}')
    if type(revision) is not int or revision < 1:
        raise ValueError('revision must be a positive integer')
    return f'{tag}-Siriusrry' if revision == 1 else f'{tag}-r{revision}-Siriusrry'
...
def select_release(release, cfg):
    ...
    return release_tag(release['tag_name'], cfg['revision'])


def plan(repo):
    cfg = config()
    upstream = gh_json(f"repos/{cfg['upstream']}/releases/latest")
    tag = select_release(upstream, cfg)
    ...
    if response.returncode == 0:
        ...
        if not existing['draft']:
            ...
            for key in ['upstream_sha', 'fingerprint', 'version']:
                if info[key] != plan[key]:
                    raise ValueError(f'Published {tag} has different {key}; bump build-config revision')
            plan['build'] = False
...
def prepare():
    ...
    for patch in patches:
        # No auto conflict resolution or context-free fuzzy replacement.
